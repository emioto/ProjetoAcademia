...
(function(factory) {
    ...
    EvoCalendar = (function() {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.$cal_days_labels = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab'];
			...
			_.$cal_months_labels_br = ['Janeiro', 'Fevereiro', 'Março', 'Abril',
    ...
    EvoCalendar.prototype.init = function(creation) {
        ...
        if (!$(_.$calendar).hasClass('calendar-initialized')) {
            ...
            $(_.$calendar).addClass('evo-calendar calendar-initialized event-hide sidebar-hide');
    ...
    EvoCalendar.prototype.buildCalendar = function(val, new_month, new_year) {
		...
		var nomemes =  _.$cal_months_labels_br[new_month];
        ...
        function buildMainHTML() {
            ...
            if(_.options.eventListToggler) {
                mainHTML += '<span id="eventListToggler" title="Recolher/Expandir"><button class="icon-button"><span class="chevron-arrow-right"></span></button></span>';
            }
            _.$mainHTML = mainHTML;
        }
        ...
        function buildSidebarHTML() {
            ...
            for(var i = 0; i < _.$cal_months_labels.length; i++) {
                ...
                sidebarHTML += '" month-val="'+i+'">'+_.$cal_months_labels_br[i]+'</li>';
            }
            sidebarHTML += '</ul>';
            if(_.options.sidebarToggler) {
                sidebarHTML += '<span id="sidebarToggler" title="Close sidebar"><i class="fa fa-calendar fa-2x" aria-hidden="true"></i></span>';
            }
        ...
        function buildEventListHTML() {
            if(_.options.calendarEvents != null) {
				var eventHTML = '<div class="event-header"><p>' + (_.$active_day_el == null ? _.$active_date.substring(3, 5) : _.$active_day_el[0].innerText) + ' ' +_.$cal_months_labels_br[_.$active_month] + ' de ' + _.$active_year + '</p></div>';		
				var hasEventToday = false;
                eventHTML += '<div>';
                
                if(_.options.calendarEvents.length > 0) 
                {
                    var idTreino = 0;
                    for (var i = 0; i < _.options.calendarEvents.length; i++) {
                        if(_.$active_date === _.options.calendarEvents[i].date) {
                            ...
                            idTreino = _.options.calendarEvents[i].idtreino;
                    ...
                    {
                        eventHTML += '<div class="event-container">';
                        eventHTML += '<div class="wrap-login100-form-btn"><div class="login100-form-bgbtn"></div><button id="btnCarregarTreino" onclick="ObterTreinoDia('+ idTreino +',\'' + _.$active_date + '\')" class="login100-form-btn">Ver treino completo</button></div>';
                        eventHTML += '</div>';
                    }
                }

                if(!hasEventToday) {
                    eventHTML += '<p>Voce não tem treino hoje! Aproveite para aprender sobre as variaveis e métodos! :)</p>';
                }
